...
class Booking extends REST_Controller {
		...
		if($token == TOKEN)
		{
            ...
            else
            {
				...
				/*********************************************** */
					...
					foreach($adminSetting as $setting)
					{
						if($setting['commission_type']=='Admin')
						{
							...
							$admin_commision=($adminper/100)*$total;
						}
						if($setting['commission_type']=='GST')
						{
							$gstper=$setting['commission'];
							$gst_amount=($gstper/100)*$total;
						}
					}
				...
				/*********************************************** */
					...
					$paymentDetails['admin_commision'] = round($admin_commision);
					$paymentDetails['admin_percentage'] = $adminper;

					$paymentDetails['gst_percentage'] = $gstper;
					$paymentDetails['gst_amount'] = round($gst_amount);
		...
		if($token == TOKEN)
		{
            ...
            else
            {
				...
				{ 
					...
					// $data['order_no']="BJO$order_no";
					// $data['booking_date']=$booking_date;
					// $Subject="New booking placed";
					// $mailbody = $this->Common_Model->email_content('Booking',$data);
					// $usermail=$this->Common_Model->SendMail($userDetails->email,$mailbody,$Subject);

					$mailMsg="Hello ".$userDetails->full_name.",<br><br>";
					$mailMsg.="Your new booking no BJO$order_no successfully placed<br>";
					$mailMsg.="<h3>Booking Information</h3>";
					$mailMsg.="Order No : BJO$order_no<br>";
					$mailMsg.="Booking Date : $booking_date<br>";
					$mailMsg.="Duration : $duration<br>";
					$mailMsg.="Time : $time_slot<br>";
					$this->Common_Model->SendMail($userDetails->email,$mailMsg,$title);
	...
	public function ccavenuepayment_post()
	{
		...
		if($token == TOKEN)
		{
			if(isset($amount) && $amount>0)
			{
				...
				$redirect_url = 'https://bhooljao.csns.info/api/customer/Booking/status_payment';
				$cancel_url = 'https://bhooljao.csns.info/api/customer/Booking/status_payment';
				// $cancel_url = 'https://bhooljao.csns.info/api/customer/Booking/cancel_payment';
				$thankyou_url = 'https://bhooljao.csns.info/thankyou.php';
				...
				$access_code = 'AVBU38KL66AP92UBPA';	//Shared by CCAVENUES
				$working_key = 'C89934E0288B5DC06997D2BF5B32F022';	//Shared by CCAVENUES
	...
	public function status_payment_post()
	{
		...
		// {
			...
			header("Location: https://bhooljao.csns.info/thankyou.php"); 
			exit;
